...
export const paymentInitializer = async (paymentInfo: any) => {
  ...
  try {
    const res = await axios.post(config.payment_url!, {
      ...
      currency: "BDT",
      success_url: `https://travel-vista-new-version-server.vercel.app/api/payment/confirmation?tran_id=${tran_id}&status=success`,

      fail_url: `https://travel-vista-new-version-server.vercel.app/api/payment/confirmation?tran_id=${tran_id}&status=failed`,
